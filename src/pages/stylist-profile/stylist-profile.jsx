...
const StylistProfile = () => {
  ...
  const fetchStylistData = async (id) => {
    try {
      ...
      setSpecializations(specsRes.data.map(spec => mapSpecializationToText(spec.specialization)));
      fetchPortfolio(id);
    } catch (err) {
  ...
  const fetchPortfolio = async (id) => {
    try {
      const res = await axios.get(`${API_BASE_URL}/portfolio_images/stylist/${id}`);
  ...
  const fetchReviews = async (id) => {
    try {
      ...
      const stylistReviews = res.data.filter(review => review.stylist_ID === parseInt(id));
  ...
  return (
    <main className="max-w-7xl mx-auto">
      ...
      <ProfileHeader 
        ...
        onBookClick={() => document.getElementById('booking-form').scrollIntoView()}
      />
